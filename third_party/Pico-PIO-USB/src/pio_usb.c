...
static PIO pio_from_num(uint8_t pio_num) {
  if (pio_num == 0) return pio0;
  if (pio_num == 1) return pio1;
#if NUM_PIOS > 2
  if (pio_num == 2) return pio2;
#endif
  return pio1;
}
...
static void apply_config(pio_port_t *pp, const pio_usb_configuration_t *c,
                         root_port_t *port) {
  pp->pio_usb_tx = pio_from_num(c->pio_tx_num);
  pp->sm_tx = c->sm_tx;
  pp->tx_ch = c->tx_ch;
  pp->pio_usb_rx = pio_from_num(c->pio_rx_num);
  pp->sm_rx = c->sm_rx;
  pp->sm_eop = c->sm_eop;
